...
describe('Get', () => {
	...
	describe('posts', () => {
		...
		it('should send correct json response if posts exist in database', async () => {
			...
			jest.spyOn(postService, 'getPostFromDB').mockResolvedValue([postMockData]);
			jest.spyOn(postService, 'postCount').mockResolvedValue(1);

			await Get.prototype.posts(req, res);
			expect(postService.getPostFromDB).toHaveBeenCalledWith({}, 0, 10, {
				createdAt: -1
			});
			expect(res.status).toHaveBeenCalledWith(200);
			expect(res.json).toHaveBeenCalledWith({
		...
		it('should send empty posts', async () => {
			...
			jest.spyOn(PostCache.prototype, 'getPostsFromCache').mockResolvedValue([]);
			jest.spyOn(PostCache.prototype, 'getTotalPostsInCache').mockResolvedValue(0);
			jest.spyOn(postService, 'getPostFromDB').mockResolvedValue([]);
	...
	describe('postWithImages', () => {
		...
		it('should send correct json response if posts exist in database', async () => {
			...
			await Get.prototype.postsWithImages(req, res);
			expect(postService.getPostFromDB).toHaveBeenCalledWith(
				{ imgId: '$ne', gifUrl: '$ne' },
				0,
				10,
				{ createdAt: -1 }
